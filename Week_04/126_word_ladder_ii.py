...
"""
通过广度优先遍历来寻找最短路径：
1. 首先用哈希表保存所有结点当作候选结点集，从里面找出 beginWord 的可达结点，将可达结点同样用哈希表保存形成可达结点集。
2. 当进行下一层可达结点搜索时，将该层的可达结点从候选结点中删除，保证下一层搜索的可达结点不会是该层的结点，保证路径最短。
3. 当 endWord 出现在可达结点集中说明找到了最短路径，不需要再进行下一层的搜索，然后遍历该层中能够到达 endWord 的所有路径并返回。
"""
